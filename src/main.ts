...
function isHardFiltered(candidate: Candidate, config: AppConfig): string | undefined {
  if (candidate.content.trim().length < config.minimumContentLength) return 'content-too-short'
  return undefined
}

function candidateExclusion(candidate: Candidate, config: AppConfig, now: Date, sourceCheckpoint?: string): 'too-old' | 'before-checkpoint' | undefined {
  if (!candidate.publishedAt) return sourceCheckpoint ? 'before-checkpoint' : undefined
  const publishedAt = Date.parse(candidate.publishedAt)
  if (!Number.isFinite(publishedAt) || now.getTime() - publishedAt > config.maxItemAgeHours * 3_600_000) return 'too-old'
  return sourceCheckpoint && publishedAt <= Date.parse(sourceCheckpoint) ? 'before-checkpoint' : undefined
}
...
export async function runPipeline(options: RunOptions): Promise<PipelineSummary> {
  ...
  const sourceUrlById = new Map(config.sourceUrls.map((source) => [source.id, source.url]))
  let tooOld = 0
  let beforeCheckpoint = 0
  const eligibleCandidates = [...candidates.values()].filter((candidate) => {
    const sourceUrl = sourceUrlById.get(candidate.sourceId)
    const exclusion = candidateExclusion(candidate, config, now, sourceUrl ? state.sourceCheckpoints[sourceUrl] : undefined)
    if (exclusion === 'too-old') tooOld += 1
    if (exclusion === 'before-checkpoint') beforeCheckpoint += 1
    return exclusion === undefined
  })
  const orderedCandidates = eligibleCandidates.sort((left, right) => {
  ...
  const failedSourceIds = new Set(collection.errors.map((error) => error.sourceId))
  for (const candidate of candidatesForRun) {
    ...
    if ((existingDecision && existingDecision.status !== 'failed') || publishedKeys.has(decisionKey) || seenKeys.has(decisionKey)) {
      alreadyDecided += 1
      continue
    }
    ...
    const filterReason = !options.allowFixtureSources && isReservedFixtureSource(candidate)
      ...
      : isHardFiltered(candidate, config)
    if (filterReason) {
      filtered += 1
  ...
  for (const source of config.sourceUrls) {
    if (!failedSourceIds.has(source.id)) state.sourceCheckpoints[source.url] = now.toISOString()
  }
  if (failedSourceIds.size === 0) state.lastRunAt = now.toISOString()
  pruneDecisionState(state, config.maxDecisionRecords)
  await saveState(config.statePath, state)
  return {
    ...
    tooOld,
    beforeCheckpoint,
    selected: candidatesForRun.length,
    skipped,
    alreadyDecided,
    filtered,
    gateEvaluated,
